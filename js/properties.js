...
export function showProperties(elem) {
    ...
    // Color group
    addGroup(containerEl, 'Farbe', () => {
        const frag = document.createDocumentFragment();
        frag.appendChild(makeRow('Farbe', colorInput(elem.props.color || '#000000', v => onChangeFn(elem.id, 'color', v))));
        return frag;
    });
...
function colorInput(value, onChange) {
    ...
    wrap.className = 'prop-color-container';

    const colorPicker = document.createElement('input');
    colorPicker.type = 'color';
    colorPicker.className = 'prop-color-picker';
    colorPicker.value = (value && /^#[0-9A-Fa-f]{6}$/.test(value)) ? value : '#000000';

    const textInput = document.createElement('input');
    textInput.type = 'text';
    textInput.className = 'prop-input prop-color-text';
    ...
    const palette = ['#000000', '#2563eb', '#dc2626', '#16a34a', '#d97706', '#9333ea', '#64748b'];
    palette.forEach(c => {
